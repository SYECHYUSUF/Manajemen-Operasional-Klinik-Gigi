  ...
  return (
    <div className="space-y-8 animate-in fade-in-50 duration-500 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-extrabold text-[#0D5A94]">Laporan Klinik</h2>
          <p className="text-slate-500 mt-1 text-sm">Ringkasan kinerja keuangan dan operasional klinik.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 h-10 px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 transition-colors"
            >
              ...
              <div className="absolute right-0 top-full mt-1 w-40 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-100 dark:border-slate-800 z-20 py-1 overflow-hidden">
                {PERIODE.map(p => (
                  <button key={p} onClick={() => { setPeriode(p); setShowPeriode(false); }}
                    className={`w-full px-4 py-2 text-sm text-left transition-colors ${periode === p ? "bg-[#0D5A94]/10 text-[#0D5A94] font-bold" : "text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800"}`}
                  >{p}</button>
                ))}
              </div>
            )}
        ...
        {[
          {
            ...
            icon: DollarSign, warna: "text-[#0D5A94] bg-blue-50 dark:bg-blue-900/20",
          ...
          },
          {
            ...
            icon: CalendarCheck, warna: "text-purple-600 bg-purple-50 dark:bg-purple-900/20",
            spark: MONTHLY.map(m => m.janji),
          },
          {
            judul: "Tagihan Pending", nilai: "3",
            naik: false, selisih: "Perlu ditindak",
            icon: AlertTriangle, warna: "text-amber-600 bg-amber-50 dark:bg-amber-900/20",
            spark: [5, 3, 7, 4, 6, 3],
          },
        ].map((s, i) => (
          <Card key={i} className="border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
            <CardContent className="p-5">
              ...
              <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">{s.judul}</p>
                  ...
                  <div key={i}>
                    <div className="flex items-center justify-between mb-1.5">
                      <div className="flex items-center gap-2">
                        <span className="w-5 h-5 rounded-full bg-slate-100 dark:bg-slate-800 text-[10px] font-extrabold text-slate-500 flex items-center justify-center">{i + 1}</span>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-xs text-slate-500">{l.jumlah}x</span>
                      </div>
                    </div>
      ...
      <Card className="border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/50">
          <div className="flex items-center gap-2">
            <FileText className="h-4 w-4 text-[#0D5A94]" />
