  ...
  return (
    <div className="space-y-8 animate-in fade-in-50 duration-500">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 mb-8">
        <div>
          <h2 className="text-3xl font-extrabold text-[#0D5A94]">Clinic Overview</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <Card className="border-slate-100 shadow-sm shadow-slate-200/50">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="p-2 bg-blue-50 rounded-lg text-[#0D5A94]">
        ...
        <Card className="border-slate-100 shadow-sm shadow-slate-200/50">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="p-2 bg-teal-50 rounded-lg text-[#006b57]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <Card className="col-span-1 lg:col-span-8 border-slate-100 shadow-sm overflow-hidden flex flex-col">
          <CardHeader className="p-6 border-b border-slate-100 flex flex-row items-center justify-between space-y-0 pb-4">
            <div>
              <CardTitle className="text-xl font-bold text-[#0D5A94]">Recent Appointments</CardTitle>
              <p className="text-xs text-slate-400 mt-1">Live view of current and upcoming sessions</p>
            </div>
            <Button variant="link" className="text-[#0D5A94] font-bold">View All</Button>
          </CardHeader>
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-slate-50">
                {recentAppointments.length === 0 ? (
                  ...
                  return (
                    <tr key={apt.id} className="hover:bg-teal-50/30 transition-colors even:bg-slate-50 dark:even:bg-slate-800">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <Avatar className="h-8 w-8 bg-blue-100 text-[#0D5A94]">
                            <AvatarFallback className="text-[10px] font-bold bg-blue-100 text-[#0D5A94]">
                              {getInitials(apt.patient?.full_name || "U K")}
                            </AvatarFallback>
                          </Avatar>
                          <span className="font-semibold text-slate-700">{apt.patient?.full_name}</span>
                        </div>
                      </td>
        ...
        <div className="col-span-1 lg:col-span-4 space-y-6">
          ...
          <Card className="border-slate-100 shadow-sm">
            ...
            <CardContent className="p-6 pt-0 space-y-4">
               {[
                 ...
                 <div key={i} className={`flex items-center justify-between p-3 rounded-lg bg-slate-50 border-l-4 ${item.urgent ? 'border-red-500' : 'border-amber-400'}`}>
                    <div>
          ...
          <div className="relative group h-40 rounded-xl overflow-hidden cursor-pointer bg-slate-900">
             ...
             <div className="absolute inset-0 bg-slate-800 group-hover:scale-105 transition-transform duration-500 flex items-center justify-center">
                 <Settings className="w-16 h-16 text-slate-700 opacity-50" />
             </div>
          </div>
