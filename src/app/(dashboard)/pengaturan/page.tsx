...
export default function PengaturanAplikasiPage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in-50 duration-500 max-w-5xl mx-auto">
      <div>
        <h2 className="text-3xl font-extrabold text-[#0D5A94] dark:text-blue-400">Pengaturan Aplikasi</h2>
        <p className="text-slate-500 dark:text-slate-400 mt-1">Konfigurasi profil klinik, preferensi tampilan, dan sistem.</p>
      </div>
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-semibold transition-all duration-150 ${
                activeTab === tab.id
                  ? "bg-white dark:bg-slate-900 text-[#0d5a94] shadow-md shadow-slate-200/60"
      ...
      <Card className="border-0 shadow-lg shadow-slate-200/50">
        <CardContent className="p-6 sm:p-8">
          {activeTab === "profil" && (
            <div className="space-y-8 animate-in fade-in-50 slide-in-from-bottom-2 duration-300">
              <div className="flex items-start gap-6">
                <div className="h-24 w-24 shrink-0 rounded-2xl bg-slate-50 dark:bg-slate-800 border-2 border-dashed border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center text-slate-400 hover:bg-slate-100 dark:bg-slate-800 transition-colors cursor-pointer">
                  <Upload className="h-6 w-6 mb-1" />
                  <span className="text-[10px] font-bold">Ubah Logo</span>
                </div>
                <div className="flex-1 space-y-1.5">
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Nama Klinik</label>
                  <input
                    type="text"
                    defaultValue="DentalCloud Pro Clinic"
                    className="w-full px-4 h-11 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-[#0d5a94]/20 focus:bg-white dark:bg-slate-900 transition-all shadow-sm"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Email Resmi</label>
                  <div className="relative">
                    <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-300" />
                    <input
                      type="email"
                      defaultValue="hello@dentalcloud.id"
                      className="w-full pl-10 pr-4 h-11 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-[#0d5a94]/20 focus:bg-white dark:bg-slate-900 transition-all shadow-sm"
                    />
                  </div>
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Nomor Telepon</label>
                  <div className="relative">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full pl-10 pr-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-[#0d5a94]/20 focus:bg-white dark:bg-slate-900 transition-all resize-none shadow-sm"
                  />
                </div>
              </div>
            </div>
          ...
          {activeTab === "tampilan" && mounted && (
            <div className="space-y-6 animate-in fade-in-50 slide-in-from-bottom-2 duration-300">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {/* Theme cards */}
                <div onClick={() => setTheme('light')} className={`p-4 rounded-xl border-2 cursor-pointer transition-all ${theme !== 'dark' ? 'border-[#0d5a94] bg-blue-50 dark:bg-blue-900/20/50 dark:bg-blue-900/20' : 'border-slate-100 dark:border-slate-800 hover:border-slate-200 dark:border-slate-700 dark:hover:border-slate-700'}`}>
                ...
                <div onClick={() => setTheme('dark')} className={`p-4 rounded-xl border-2 cursor-pointer transition-all ${theme === 'dark' ? 'border-[#0d5a94] bg-blue-50 dark:bg-blue-900/20/50 dark:bg-blue-900/20' : 'border-slate-100 dark:border-slate-800 hover:border-slate-200 dark:border-slate-700 dark:hover:border-slate-700'}`}>
                ...
                <div onClick={() => setTheme('system')} className={`p-4 rounded-xl border-2 cursor-pointer transition-all ${theme === 'system' ? 'border-[#0d5a94] bg-blue-50 dark:bg-blue-900/20/50 dark:bg-blue-900/20' : 'border-slate-100 dark:border-slate-800 hover:border-slate-200 dark:border-slate-700 dark:hover:border-slate-700'}`}>
              ...
              {['Senin - Jumat', 'Sabtu', 'Minggu'].map((day, i) => (
                <div key={i} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 rounded-xl">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold text-slate-700 dark:text-slate-300 w-32">{day}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <input type="time" defaultValue={i === 2 ? "" : "08:00"} disabled={i === 2} className="px-3 py-1.5 bg-white dark:bg-slate-900 rounded-lg text-sm font-medium shadow-sm outline-none focus:ring-2 focus:ring-[#0d5a94]/20 disabled:opacity-50" />
                    <span className="text-slate-400 font-medium">-</span>
                    <input type="time" defaultValue={i === 2 ? "" : i === 1 ? "14:00" : "20:00"} disabled={i === 2} className="px-3 py-1.5 bg-white dark:bg-slate-900 rounded-lg text-sm font-medium shadow-sm outline-none focus:ring-2 focus:ring-[#0d5a94]/20 disabled:opacity-50" />
                  </div>
                </div>
              ))}
            </div>
          ...
          {activeTab === "integrasi" && (
            <div className="space-y-4 animate-in fade-in-50 slide-in-from-bottom-2 duration-300">
              <div className="p-5 border border-[#0d5a94]/20 bg-blue-50 dark:bg-blue-900/20/50 rounded-xl flex items-center justify-between shadow-sm">
                ...
                <div className="px-3 py-1 bg-[#006b57]/10 text-[#006b57] dark:text-green-400 text-xs font-bold rounded-full border border-[#006b57]/20">Aktif</div>
              </div>
              
              <div className="p-5 border border-slate-100 dark:border-slate-800 rounded-xl flex items-center justify-between shadow-sm hover:border-slate-200 dark:border-slate-700 transition-colors">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Kirim pengingat jadwal otomatis via WhatsApp.</p>
                  </div>
                </div>
